...
const options: { value: SinalRitmo; label: string; icon: string; color: string }[] = [
  { value: 'tudo_certo', label: 'Certo', icon: 'thumb_up', color: 'positive' },
  { value: 'boiando', label: 'Boiando', icon: 'sentiment_very_dissatisfied', color: 'warning' },
  { value: 'muito_rapido', label: 'Rápido', icon: 'speed', color: 'negative' },
  { value: 'muito_devagar', label: 'Lento', icon: 'slow_motion_video', color: 'info' },
];
...
<template>
  <div class="tw-w-full">
    <div class="tw-text-center tw-mb-3">
      <h3 class="tw-text-base tw-font-bold tw-text-primary">Termômetro de Ritmo</h3>
      <p class="tw-text-xs text-muted">Como está a aula para você agora?</p>
    </div>
    ...
    <div class="tw-grid tw-grid-cols-4 tw-gap-2">
      <q-btn
        ...
        class="tw-p-2 tw-rounded-lg active:tw-scale-95 tw-transition-all"
        no-caps
        unelevated
